...
def main():
    ...
    # move/delete files from s3 accordingly. if false, stop script
    if s3Mover():
        print("moving files")
    else:
        return "no new files"
    ...
    for da_id in processing_list:
        ...
        if isActiveRefresher(da_id):
            ...
            if row == []:
                ...
                moveFiles(da_id)
                print(da_id + " first delivery, logged and moved")
...
def s3Mover():
    ...
    process1 = subprocess.Popen(syncS3Command.split(), stdout=subprocess.PIPE)
    ...
    process2 = subprocess.Popen(cleanS3Command.split(), stdout=subprocess.PIPE)
...
def moveFiles(da_id):
    ...
    test code:
def moveFiles(da_id):
    src = "/Users/kwei/Desktop/asdf/"+str(da_id)
    dest = "/Users/kwei/Desktop/"+str(da_id)
    if os.path.exists(dest):
        files = os.listdir(src)
        for f in files:
            if f.endswith(('.gz', '.md5', '.csv')):
        ...
        shutil.rmtree(src)
          ...
          shutil.move(src, "/Users/kwei/Desktop/")
    '''
...
def cleanFolder(path):
    ...
    #if there are more than 1 taxonomy files, get rid of the old one, if there are multiple data files, append them.
    ...
    '''testcode:
    path = "/home/kwei/mschf/daid_428936/"
    da_id = 75371336
    path = path + str(da_id) + "/" + "test/"
    gzips = glob.glob(path+"*.txt")
    '''
    ...
    for datafile_to_remove in datafiles:
        os.remove(datafile_to_remove)

    # taxonomyCleaner(da_id)
    ...
    import os
    import glob
    def removeTaxos(da_id):
        taxos = glob.glob("/Users/kwei/Desktop/testingfolder/"+str(da_id)+"/*.csv")
        if len(taxos) > 1:
            taxos.sort(key=os.path.getctime)
            for taxo in taxos[:-1]:
                os.remove(taxo)
...
def finalmd5(path):
    ...
    if len(data) == 1:
        ...
        md5er = "md5sum " + master_file_path + " > " + master_file_path + ".md5"
...
def sendsftp(path, da_id):
    ...
    for f in files:
        if f.endswith(('.gz', '.md5', '.csv')):
            pathsToBeMoved.append(path + f)

    for path in pathsToBeMoved:
        transferred = False
        while transferred == False:
            print ("trying to transfer " + path)
            string = os.popen('scp -q -oIdentityFile=/apps/file_deliverer/current/config/dist_id_rsa ' + path + ' 7898fxsv@sfgext.acxiom.com:/ && echo success!').read()
            print string
